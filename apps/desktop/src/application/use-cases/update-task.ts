...
export interface UpdateTaskInput { readonly taskId: string; readonly title: string; readonly priority: Task["priority"]; readonly assigneeId: string | null; }
...
export class UpdateTask {
  ...
  async execute(input: UpdateTaskInput): Promise<Task> {
    ...
    const updated = { ...current, title, priority: input.priority, assigneeId: input.assigneeId, updatedAt: this.clock.now() };
